...
public class TripleIndex {
	...
	/**
	 * 查询索引。三元组查询：SPO，即subject、predicate、object
	 * @param subject
	 * @param predicate
	 * @param object
	...
	public List<Triple> search(String subject, String predicate, String object, int maxNumberOfResults) {
		...
		try {
			...
			if (subject != null) {
				...
				bq.add(tq, BooleanClause.Occur.MUST); //主语是must
			}
			if (predicate != null) {
				Query tq = new TermQuery(new Term(FIELD_NAME_PREDICATE, predicate));
				bq.add(tq, BooleanClause.Occur.MUST); //谓语也是must
			}

			if (object != null && object.length() > 0) {
				Query q = null;
				if (urlValidator.isValid(object)) { //判断url是否合法，合法则直接构建term查询
