...
public class CandidateUtil {
	 ...
	 * 
	 * @param graph
	 * @param document 表示要进行linking的文本的信息
	 * @param threshholdTrigram
	 * @param heuristicExpansionOn
	 * @throws IOException
	 */
	public void insertCandidatesIntoText(DirectedSparseGraph<Node, String> graph, Document document,
		...
		// used for heuristic label expansion start with longest Named Entities，按照长度从大到小排序
	...
	/**
	 * 启发式、探索式扩展，从heuristicExpansion中找出包含label，同时长度最短的String作为label的扩展
	 * @param heuristicExpansion
	 * @param label
	 * @return
	 */
	private String heuristicExpansion(HashSet<String> heuristicExpansion, String label) {
		...
		for (String key : heuristicExpansion) {
			if (key.contains(label)) { //要求key必须包含label字符串
				// 选取长度最短的包含label的字符串作为Expansion
				...
				// tmp就是label，那么只要key包含label，且不是label，就更新tmp
				if (tmp.length() < key.length() && tmp == label) {
					tmp = key;
					expansion = true;
					log.debug("Heuristic expansion: " + label + "-->" + key);
	...
	private void checkLabelCandidates(DirectedSparseGraph<Node, String> graph, double threshholdTrigram,
		...
		// searchByAcronym，首字母缩略词搜索
		if (acronym == true) {
			//这里的前提是长度不大于4，而且都是大写字母
		...
		if (countFinalCandidates == 0) {
			...
			for (Triple c : candidates) {
				...
				if (candidateURL.startsWith(nodeType)) {
					...
					if (c.getPredicate().equals("http://www.w3.org/2000/01/rdf-schema#label")) {
						//Here we set the similarity as maximum because rfds:label refers to the main refrence of a given resource
						if (nGramDistance.getDistance(surfaceForm, label) < 1.0) {
							continue;
						}
					} else if (!c.getPredicate().equals("http://www.w3.org/2000/01/rdf-schema#label")) { 
						//Here the similarity is in accordance with the user's choise
					...
					if (commonEntities == true) { // Being able to get all kinds of resource not only Person, Organization, Location
			...
			if (!added && !searchInSurfaceForms && context == true) {
				...
				//looking for all entities together
