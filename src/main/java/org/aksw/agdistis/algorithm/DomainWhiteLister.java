...
public class DomainWhiteLister {
	...
	public DomainWhiteLister(TripleIndex index) throws IOException {
		...
		iswikidata = Boolean.valueOf(prop.getProperty("iswikidata"));
	...
	public boolean fitsIntoDomain(String candidateURL) {
		List<Triple> tmp;
		if (iswikidata) {
			tmp = index.search(candidateURL, "http://www.wikidata.org/prop/direct/P31", null);
		} else {
			tmp = index.search(candidateURL, "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", null);
		}
